...
const Route = (props: RouteProps) => {
  ...
  const checkLang = useCallback(() => {
    if (potentialLangFromUrl === "" && currenti18nLang === fallbackLng) {
    ...
    } else if (avaliableLanguages.includes(potentialLangFromUrl)) {
      // proper lang in url, e.g. : "/en/login"
      potentialLangFromUrl !== currenti18nLang &&
        i18n.changeLanguage(potentialLangFromUrl); // will not change lang if someone is clicking the same change lng btn a couple of times
    } else {
      // /404
      // /login
      // /login/user/account
      ...
      if (
        potentialLangFromUrl !== NOT_FOUND_LINK_WITHOUT_SLASH &&
  ...
  }, [potentialLangFromUrl, i18n, currenti18nLang]);
